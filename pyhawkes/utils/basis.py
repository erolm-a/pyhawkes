...
class Basis(object):
    ...
    def __init__(
        self, B, dt, dt_max, orth=False, norm=False, allow_instantaneous=False
    ):
        self.B = B
        self.dt = dt
        self.dt_max = dt_max
        ...
        self.basis = self.interpolate_basis(
            self.create_basis(), self.dt, self.dt_max, self.norm
        )
    ...
    def convolve_with_basis(self, S):
        ...
        for b in np.arange(B):
            F[:, :, b] = sig.fftconvolve(
                S, np.reshape(self.basis[:, b], (R, 1)), "full"
            )[:T, :]
...
class CosineBasis(Basis):
    ...
    def __init__(
        ...
        super(CosineBasis, self).__init__(
            B, dt, dt_max, orth, norm, allow_instantaneous
        )
    ...
    def create_basis(self):
        ...
        basis_fn = (
            lambda u, c, w: (
                np.cos(np.maximum(-np.pi, np.minimum(np.pi, (u - c) * np.pi / w / 2.0)))
                + 1
            )
            / 2.0
        )
...
class IdentityBasis(Basis):
    def __init__(self, dt, dt_max, norm=True, allow_instantaneous=False):
        self.dt = dt
        self.dt_max = dt_max
        self.allow_instantaneous = allow_instantaneous
        self.norm = norm
