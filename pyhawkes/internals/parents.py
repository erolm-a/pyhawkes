...
class DiscreteTimeParents(GibbsSampling, MeanField):
    ...
    def expected_log_likelihood(self, x):
        ...
        for k2, (Sk, Fk, Tk, Zk) in enumerate(zip(self.Ss, self.Fs, self.Ts, self.Z)):
            ...
            # Compute the instantaneous log rate
            exp_lam = exp_lam0[k2] + (Fk * exp_W[:, k2][:, None] * exp_G[:, k2, :]).sum(
                axis=(1, 2)
            )
    ...
    def _mf_update_Z(self):
        ...
        for k2, (Sk, Fk, EZk) in enumerate(zip(self.Ss, self.Fs, self.EZ)):
            mf_update_Z(k2, EZk, Sk, exp_E_log_lambda0, exp_E_log_W, exp_E_log_g, Fk)
    ...
    def get_vlb(self):
        ...
        for k, (EZk, Sk, Fk) in enumerate(zip(self.EZ, self.Ss, self.Fs)):
            vlb += mf_vlb(
                k, self.T, EZk, Sk, self.Ns, E_ln_lam, E_lam, E_ln_W, E_W, E_ln_g, Fk
            )
    ...
    def get_vlb_python(self):
        ...
        for k, (EZk, Sk) in enumerate(zip(self.EZ, self.Ss)):
            ln_u0 = np.log(EZk[:, 0] / Sk.astype(np.float64))
        ...
        for k, (EZk, Sk, Fk, Tk, tk) in enumerate(
            zip(self.EZ, self.Ss, self.Fs, self.Ts, self.ts)
        ):
            E_ln_Wg = (
                np.log(Fk) + E_ln_W[:, k][None, :, None] + E_ln_g[:, k, :][None, :, :]
            )
            ...
            # Second term
            ln_u = np.log(EZk[:, 1:] / Sk[:, None].astype(np.float64))
...
class ContinuousTimeParents(GibbsSampling):
    ...
    def __init__(self, model, S, C, T, K, dt_max):
        ...
        assert C.dtype == np.int64 and (len(C) == 0 or (C.min() >= 0 and C.max() < K))
        self.S = S
        self.C = C
        self.T = T
        self.K = K
    ...
    def resample(self):
        ...
        ct_resample_Z_logistic_normal(S, C, Z, dt_max, lambda0, W, mu, tau)
        ...
        ct_compute_suff_stats(
            S, C, Z, dt_max, self.bkgd_ss, self.weight_ss, self.imp_ss
        )
