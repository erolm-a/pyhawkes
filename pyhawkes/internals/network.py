...
class _StochasticBlockModelBase(BayesianDistribution):
    ...
    def __init__(
        ...
        """
        assert isinstance(K, int) and C >= 1, (
            "K must be a positive integer number of nodes"
        )
        self.K = K

        assert isinstance(C, int) and C >= 1, (
            "C must be a positive integer number of blocks"
        )
        self.C = C
        ...
        if isinstance(pi, (int, float)):
            ...
            assert isinstance(pi, np.ndarray) and pi.shape == (C,), (
                "pi must be a sclar or a C-vector"
            )
            self.pi = pi
        ...
        if v is not None:
            if np.isscalar(v):
                ...
                assert (
                    isinstance(v, np.ndarray) and v.shape == (C, C) and np.amin(v) >= 0
        ...
        # are ignored
        if not (p is None or c is None or v is None):
            self.fixed = True
        else:
            self.fixed = False
    ...
    def log_likelihood(self, x):
        ...
        lp += (
            Beta(
                self.tau1 * np.ones((self.C, self.C)),
    ...
    def resample_c(self, A, W):
        ...
        for k in range(self.K):
            ...
            for ck in range(self.C):
                ...
                # p(A[k,k'] | c)
                lp[ck] += Bernoulli(self.p[ck, c_temp]).log_probability(A[k, :]).sum()

                # p(A[k',k] | c)
                lp[ck] += Bernoulli(self.p[c_temp, ck]).log_probability(A[:, k]).sum()

                # p(W[k,k'] | c)
                lp[ck] += (
                    A[k, :]
                    * Gamma(self.kappa, self.v[ck, c_temp]).log_probability(W[k, :])
                ).sum()

                # p(W[k,k'] | c)
                lp[ck] += (
                    A[:, k]
                    * Gamma(self.kappa, self.v[c_temp, ck]).log_probability(W[:, k])
                ).sum()
...
class MeanFieldSBM(_StochasticBlockModelBase, MeanField, MeanFieldSVI):
    ...
    def __init__(
        ...
        super(MeanFieldSBM, self).__init__(
            K=K,
            C=C,
            c=c,
            pi=pi,
        ...
        self.mf_m = np.random.dirichlet(10 * np.ones(self.C), size=(self.K,))
        ...
        for c1 in range(self.C):
            for c2 in range(self.C):
                ...
                E_p += (
                    pc1c2
                    * self.mf_tau1[c1, c2]
                    / (self.mf_tau0[c1, c2] + self.mf_tau1[c1, c2])
                )
    ...
    def expected_log_v(self):
        ...
        for c1 in range(self.C):
            for c2 in range(self.C):
                ...
                # Get the probability of a connection for this pair of classes
                E_log_v += pc1c2 * (
                    psi(self.mf_alpha[c1, c2]) - np.log(self.mf_beta[c1, c2])
                )
        return E_log_v
        ...
        for k in range(self.K):
            ...
            for ck in range(self.C):
                ...
                for cnotk in range(self.C):
                    ...
                    E_v_ck_to_cnotk += p_cnotk * (
                        self.mf_alpha[ck, cnotk] / self.mf_beta[ck, cnotk]
                    )
                    E_ln_v_ck_to_cnotk += p_cnotk * (
                        psi(self.mf_alpha[ck, cnotk]) - np.log(self.mf_beta[ck, cnotk])
                    )

                    # Expected log scale of connections from cnotk to ck
                    E_v_cnotk_to_ck += p_cnotk * (
                        self.mf_alpha[cnotk, ck] / self.mf_beta[cnotk, ck]
                    )
                    E_ln_v_cnotk_to_ck += p_cnotk * (
                        psi(self.mf_alpha[cnotk, ck]) - np.log(self.mf_beta[cnotk, ck])
                    )
                ...
                lp[ck] += (
                    ...
                    * Gamma(self.kappa).negentropy(
                        ...
                        E_ln_beta=E_ln_v_cnotk_to_ck,
                    )
                ).sum()

                # Compute expected log prob of self connection
                if self.allow_self_connections:
                    E_ln_p_ck_to_ck = psi(self.mf_tau1[ck, ck]) - psi(
                        self.mf_tau0[ck, ck] + self.mf_tau1[ck, ck]
                    )
                    E_ln_notp_ck_to_ck = psi(self.mf_tau0[ck, ck]) - psi(
                        self.mf_tau0[ck, ck] + self.mf_tau1[ck, ck]
                    )
    ...
    def mf_update_p(self, E_A, E_notA, stepsize=1.0):
        ...
        for c1 in range(self.C):
            for c2 in range(self.C):
                ...
                self.mf_tau1[c1, c2] = (1.0 - stepsize) * self.mf_tau1[
                    c1, c2
                ] + stepsize * tau1_hat
                self.mf_tau0[c1, c2] = (1.0 - stepsize) * self.mf_tau0[
                    c1, c2
                ] + stepsize * tau0_hat
    ...
    def mf_update_v(self, E_A, E_W_given_A, stepsize=1.0):
        ...
        for c1 in range(self.C):
            for c2 in range(self.C):
                ...
                self.mf_alpha[c1, c2] = (1.0 - stepsize) * self.mf_alpha[
                    c1, c2
                ] + stepsize * alpha_hat
                self.mf_beta[c1, c2] = (1.0 - stepsize) * self.mf_beta[
                    c1, c2
                ] + stepsize * beta_hat
    ...
    def meanfieldupdate(
        self, weight_model, update_p=True, update_v=True, update_m=True, update_c=True
    ):
    ...
    def get_vlb(self, vlb_c=True, vlb_p=True, vlb_v=True, vlb_m=True):
        # import pdb; pdb.set_trace()
        vlb = 0
        ...
        if vlb_p:
            vlb += (
                Beta(self.tau1, self.tau0)
                .negentropy(
                    E_ln_p=(psi(self.mf_tau1) - psi(self.mf_tau0 + self.mf_tau1)),
                    E_ln_notp=(psi(self.mf_tau0) - psi(self.mf_tau0 + self.mf_tau1)),
                )
                .sum()
            )
        ...
        if vlb_v:
            vlb += (
                ...
                .negentropy(
                    E_lambda=self.mf_alpha / self.mf_beta,
                    E_ln_lambda=psi(self.mf_alpha) - np.log(self.mf_beta),
                )
                .sum()
            )
...
class StochasticBlockModelFixedSparsity(StochasticBlockModel):
    ...
    def __init__(
        ...
        super(StochasticBlockModelFixedSparsity, self).__init__(
            K=K,
            C=C,
            p=p,
            c=c,
    ...
    def meanfieldupdate(
        self, weight_model, update_p=False, update_v=True, update_m=True, update_c=True
    ):
        assert update_p is False, "Cannot update p!"
        super(StochasticBlockModelFixedSparsity, self).meanfieldupdate(
            weight_model,
            update_p=False,
            update_v=update_v,
            update_m=update_m,
    ...
    def meanfield_sgdstep(
        ...
        assert update_p is False, "Cannot update p!"
        super(StochasticBlockModelFixedSparsity, self).meanfield_sgdstep(
            weight_model,
            minibatchfrac,
            stepsize,
            update_p=False,
    ...
    def get_vlb(self, vlb_c=True, vlb_p=False, vlb_v=True, vlb_m=True):
        assert vlb_p is False, "Cannot calculate vlb wrt p!"
        super(StochasticBlockModelFixedSparsity, self).get_vlb(
            vlb_c=vlb_c, vlb_p=False, vlb_v=vlb_v, vlb_m=vlb_m
        )
...
class ErdosRenyiModel(StochasticBlockModel):
    ...
    def __init__(
        self, K, p=None, tau0=0.1, tau1=0.1, v=None, alpha=1.0, beta=1.0, kappa=1.0
    ):
        C = 1
        c = np.zeros(K, dtype=np.int64)
        super(ErdosRenyiModel, self).__init__(
            K,
            C,
            c=c,
            p=p,
...
class LatentDistanceAdjacencyModel(ErdosRenyiModel):
    ...
    def __init__(self, K, dim=2, v=None, alpha=1.0, beta=1.0, kappa=1.0):
        super(LatentDistanceAdjacencyModel, self).__init__(
            K=K, v=v, alpha=alpha, beta=beta, kappa=kappa
        )
